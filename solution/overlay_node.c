...
void build_table()
{
    ...
    { // impl rest of dijkstras
        ...
        while (v_temp != NULL)
        {
            if (v_temp->id == D[min_index]->dst_id)
            { // remove head
                if (v_prev == NULL)
                {
                    v_head = v_head->next;
        ...
        { // for every D
            ...
            { // check every egde
                ...
                if (D[min_index]->dst_id == Edge_List.edges[j]->src_id && D[i]->dst_id == Edge_List.edges[j]->dst_id)
                {
                    ...
                    if (combined < D[i]->cost)
                    { // update if combined is lower cost
                        D[i]->cost = combined;
...
void forward_data(struct data_pkt *pkt)
{
    ...
    else if (Route_Mode == MODE_LINK_STATE)
    {
        ...
        if (pkt->hdr.dst_id != My_ID)
        {
            ...
            struct client_conn *c = get_client_from_port(&Client_List, pkt->hdr.src_port);
            ...
            if (c == NULL)
            {
                Alarm(PRINT, "overlay_node: received data for client that does not "
                      ...
                      pkt->hdr.dst_id, pkt->hdr.src_port);
                return;
            }
            ...
            /* Send data to client */
            struct node *n = get_node_from_id(&Node_List, out);
            struct sockaddr_in ctrl_addr = n->addr;
            ctrl_addr.sin_port = htons(ntohs(n->addr.sin_port) + 1);
            printf("ctrl_addr.sin_port = %d\n", ctrl_addr.sin_port);
...
void deliver_locally(struct data_pkt *pkt)
{
    ...
    /* Send data to client */
    bytes = sizeof(struct data_pkt) - MAX_PAYLOAD_SIZE
    + pkt->hdr.data_len;
...
bool heartbeat_eco(struct heartbeat_pkt *pkt) {
    return pkt->alive;
}

/* Respond to heartbeat message by sending heartbeat echo */
bool handle_heartbeat(struct heartbeat_pkt *pkt)
{
...
void student_lsa(struct lsa_pkt *pkt) {
    printf("%d %d %d\n", pkt->hdr.type, pkt->hdr.src_id, pkt->hdr.dst_id);
}
...
void handle_dv(struct dv_pkt *pkt)
{
    ...
    for (int i = pkt->hdr.src_id; i < pkt->hdr.dst_id; i++) {
        printf("%d ", pkt->hdr.type);
    }
    puts("");
}
